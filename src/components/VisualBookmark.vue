<template>
  <figure
    class="web-bookmark-card relative inset-0 overflow-hidden text-left transition-all duration-300 ease-out"
    :class="[bookmarkClass, horizontal ? 'w-[720px]' : 'w-120']"
  >
    <slot />
    <a
      v-if="!isLoading"
      class="glass w-full flex flex-wrap text-current no-underline hover:no-underline inset-0 transition-all duration-300 ease-out z-10"
